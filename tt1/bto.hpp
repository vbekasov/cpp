...
namespace nbt
{
    ...
    template <class T> BtNode<T>::BtNode(T n){
        val = n;
    ...
    template <class T> void BTree<T>::add_node(T n){
        ...
        {
            if (n == tmp->val) return ;
            if (n < tmp->val && tmp->left != nullptr)
                tmp = tmp->left;
            else if (n > tmp->val && tmp->right != nullptr)
                tmp = tmp->right;
            else
                flag = false;
        }
    ...
    template<class T> void BTree<T>::print_tree(){
        std::cout<< "===Tree===\n";
        sup_print(this->root);
        std::cout<< "==========\n";
    }

    template<class T> void BTree<T>::sup_print(BtNode<T>* node){
    ...
    template<class T> std::vector<T> BTree<T>::tree_to_vec(){
        std::vector<T> ret;
    ...
    template<class T> void BTree<T>::sup_vec(BtNode<T>* node, std::vector<T>& vec_add){
    ...
    template<class T> void BTree<T>::sup_del(BtNode<T>* node){
        if (node->left)  sup_del(node->left);
        if (node->right) sup_del(node->right);
        delete node;
    }
    
    template<class T> BtNode<T>* BTree<T>::sup_ret_prev(T vl){
        BtNode<T>*  tmp  = this->root;
        bool        flag = true;
        while (flag){
            flag = false;
    ...
    template<class T> BtNode<T>* BTree<T>::sup_ret_node(T vl){
        BtNode<T>*  tmp = this->root;
        while (true){
    ...
    template<class T> void BTree<T>::arr_to_tree(T* arr, unsigned int sz){
        if (sz > 2){
            add_node(arr[sz / 2]);
